...
const Sidebar: React.FC<SidebarProps> = ({ onNavigate, currentPath }) => {
  ...
  const menuItems = [
    { icon: Building2, label: t('sidebar.dashboard'), path: '/' },
    { icon: FileText, label: t('sidebar.invoices'), path: '/invoices' },
    { icon: Upload, label: t('sidebar.uploadOcr'), path: '/invoices/upload' },
    { icon: Users, label: t('sidebar.customers'), path: '/customers' },
    { icon: Package, label: t('sidebar.articles'), path: '/articles' },
    { icon: Euro, label: t('sidebar.finances'), path: '/finances' },
    { icon: Bell, label: t('sidebar.reminders'), path: '/reminders' },
    { icon: Briefcase, label: t('sidebar.projects'), path: '/projects' },
    ...
    <div className="w-16 bg-white border-r border-gray-200 flex flex-col items-center py-6 space-y-6 absolute inset-y-0 left-0 md:relative transform -translate-x-full md:translate-x-0 transition duration-200 ease-in-out">
      ...
      {menuItems.map((item) => {
        ...
        const isActive = isDashboard
          ? currentPath === '/'
          : isInvoices
            ? currentPath === '/invoices' || (currentPath.startsWith('/invoices/') && !currentPath.startsWith('/invoices/upload'))
